    ...
    doNewMail(UID = '') {
        ...
        return this.seachUnseen((err, newMailIds, havemore) => {
            ...
            if (newMailIds || (newMailIds = UID)) {
                ...
                return async_1.eachSeries(uids, (n, next) => {
...
class qtGateImap extends events_1.EventEmitter {
    ...
    connect() {
        ...
        try {
            conn = tls_1.connect({ host: this.IMapConnect.imapServer, servername: this.IMapConnect.imapServer, port: this.port }, _connect);
        }
        catch (ex) {
            console.log(ex);
            return this.connect();
...
const imapAccountTest = (IMapConnect, CallBack) => {
    ...
    let startTime = null;
    const listenFolder = uuid_1.v4();
    const ramdomText = crypto_1.randomBytes(20);
    let timeout = null;
...
class imapPeer extends events_1.EventEmitter {
    ...
    mail(email) {
        ...
        console.log(` subject = [${buffer_1.Buffer.from(subject).toString('base64')}] this.pingUuid = [${this.pingUuid}] base64 = [${buffer_1.Buffer.from(this.pingUuid).toString('base64')}]`);
        if (subject) {
            /**
             *
             *
